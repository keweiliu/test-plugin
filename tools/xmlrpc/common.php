...
// recover input parameters
  ...
  if (isset($_GET['action']))
  {
    ...
    list ($host, $sub_path) = explode('/', $host, 2);
    ...
    $payload = preg_replace('#<base64>(.*?)</base64>#sei', "'<base64>'.base64_encode('$1').'</base64>'", $payload);
    
    $alt_payload = isset($_GET['altmethodpayload']) ? $_GET['altmethodpayload'] : '';
    $remove_ct = isset($_GET['remove_ct']) ? $_GET['remove_ct'] : 0;
    $ttdebug = isset($_GET['ttdebug']) ? $_GET['ttdebug'] : 0;
    ...
    if (isset($_GET['customizeUA']) && ($_GET['customizeUA'] == '1' || $_GET['customizeUA'] == '2'))
      $customizeUA = $_GET['customizeUA'];
    else
      $customizeUA = 0;
