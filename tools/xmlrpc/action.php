 ...
 *
 * @todo switch params for http comssion from 0,1,2 to values to be used directly
...
if(!empty($_FILES['fileupload']['name']) && !empty($_POST['forum_id']))
{
	include 'attach_upload_client.php';
	die();
}
?>
...
<?php
  ...
  if ($action)
  {
    ...
    $client->remove_ct = $remove_ct;
    $client->ttdebug = $ttdebug;
    ...
    switch ($customizeUA) {
      case 0:
        ...
        break;
      case 1:
        $client->customizeUA = 'Tapatalk;';
        break;
      case 2:
        $client->customizeUA = 'BYO;';
        break;
    }
...
?>
<?php
if(!isset($_GET['cookie_action']) && (($_GET['method'] == 'login') || ($_GET['method'] == 'login_mod') || ($_GET['method'] == 'sign_in')))
	$_SESSION['cookie_action'] = $GLOBALS['_xh']['cookies'];
if(in_array($_GET['method'],array('register','forget_password','logout_user')))
{
	$_SESSION['cookie_action'] = '';
}
if(isset($_GET['cookie_action']))
{
	...
	$str = '';
	$i=0;
	foreach ($_SESSION['cookie_action'] as $key => $cookie)
	{
		$str .= $key.'='.$cookie['value'];
		$i++;
		if($i != count($_SESSION['cookie_action']))
		{
			$str.=',';
		}
