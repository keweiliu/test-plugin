...
Class MbqRdEtForumTopic extends MbqBaseRdEtForumTopic {
     ...
     * $mbqOpt['case'] = 'byTopicRecords' means get data by topicRecords.$var is the topicRecords.
    ...
    public function getObjsMbqEtForumTopic($var, $mbqOpt) {
        ...
        /* global common end */
        if ($mbqOpt['case'] == 'byForum') { //will cause mark forum read !!!
            $oMbqEtForum = $var;
            if ($mbqOpt['oMbqDataPage']) {
                $oMbqDataPage = $mbqOpt['oMbqDataPage'];
                //ref forumdisplay.php START MAIN SCRIPT
// #######################################################################
// ######################## START MAIN SCRIPT ############################
// #######################################################################
$foruminfo = $var->mbqBind['forumInfo'];    //wztmdf 20131005
...
$pagenumber = $oMbqDataPage->curPage;
$daysprune = -1;
$sortfield = "lastpost";
...
if (!verify_forum_password($foruminfo['forumid'], $foruminfo['password'], false)) {     //wztmdf 20131005
...
if ($foruminfo['cancontainthreads'])
{
	...
	$sortfield = 'lastpost';    //wztmdf 20131005
	$vbulletin->GPC['sortorder'] = 'desc';  //wztmdf 20131005
    ...
    //wztmdf 20131005 begin
    if ($mbqOpt['notIncludeTop']) {
        $exttMbqNotIncludeTop = true;
        $exttMbqSubSqlStick = 'sticky = 0';
    } elseif ($mbqOpt['top']) {
        $exttMbqTop = true;
        $exttMbqSubSqlStick = 'sticky = 1';
    } else {
	...
	$oMbqDataPage->totalNum = $totalthreads;    //wztmdf 20131005
	...
	$threads = $db->query_read_slave("
		SELECT $votequery $previewfield
			thread.threadid, thread.title AS threadtitle, thread.forumid, thread.firstpostid as firstpostid, pollid, open, postusername, postuserid, thread.iconid AS threadiconid,
...
//eval('print_output("' . fetch_template('FORUMDISPLAY') . '");');
                /* common begin */
                $mbqOpt['case'] = 'byTopicRecords';
                $mbqOpt['oMbqDataPage'] = $oMbqDataPage;
                return $this->getObjsMbqEtForumTopic($exttMbqTopicRecords, $mbqOpt);
                /* common end */
            }
        } elseif ($mbqOpt['case'] == 'byTopicIds') {
            $exttMbqTopicRecords = array();
            foreach ($var as $topicId) {
                if ($thread = fetch_threadinfo($topicId)) {
                    $oMbqRdEtForum = MbqMain::$oClk->newObj('MbqRdEtForum');
                    if ($oMbqEtForum = $oMbqRdEtForum->initOMbqEtForum($thread['forumid'], array('case' => 'byForumId'))) {
                        $thread = process_thread_array($thread, $oMbqEtForum->mbqBind['lastread'], $oMbqEtForum->mbqBind['forumInfo']['allowicons']);
                        $thread['threadtitle'] = $thread['title'];  //maintain data consistency
                        $exttMbqTopicRecords[] = $thread;
                    }
                }
            }
            ...
            return $this->getObjsMbqEtForumTopic($exttMbqTopicRecords, $mbqOpt);
            /* common end */
        } elseif ($mbqOpt['case'] == 'byTopicRecords') {
            $exttMbqTopicRecords = $var;
            /* common begin */
            ...
            $forumIds = array();
            $topicIds = array();
            foreach ($exttMbqTopicRecords as $topicRecord) {
                $objsMbqEtForumTopic[] = $this->initOMbqEtForumTopic($topicRecord, array('case' => 'topicRecord'));
            }
            $sqlInString = '';
            foreach ($objsMbqEtForumTopic as $oMbqEtForumTopic) {
                ...
                $sqlInString .= ($sqlInString ? ','.$oMbqEtForumTopic->mbqBind['topicRecord']['lastpostid'] : $oMbqEtForumTopic->mbqBind['topicRecord']['lastpostid']);
            }
            //make $oMbqEtForumTopic->lastReplyAuthorId and topicContent and shortContent properties from last post
            $oMbqRdEtForumPost = MbqMain::$oClk->newObj('MbqRdEtForumPost');
        	$bbcode_parser =& new vB_BbCodeParser($vbulletin, fetch_tag_list(), false);
        	if ($sqlInString) {
                $resultLastPost = $db->query_read_slave("
                    SELECT post.*, postparsed.pagetext_html FROM " . TABLE_PREFIX . "post as post LEFT JOIN " . TABLE_PREFIX . "postparsed AS postparsed ON (postparsed.postid = post.postid AND postparsed.styleid = " . intval(STYLEID) . " AND postparsed.languageid = " . intval(LANGUAGEID) . ") WHERE post.postid in ($sqlInString)
            	");
            	while ($lastPost = $db->fetch_array($resultLastPost)) {
            	    foreach ($objsMbqEtForumTopic as &$oMbqEtForumTopic) {
            	        if ($lastPost['threadid'] == $oMbqEtForumTopic->topicId->oriValue) {
            	            $oMbqEtForumTopic->lastReplyAuthorId->setOriValue($lastPost['userid']);
            	            $lastReplyUserIds[$lastPost['userid']] = $lastPost['userid'];
                            $oMbqEtForumTopic->topicContent->setOriValue(MbqMain::$oMbqCm->exttNativeStrToUtf8($lastPost['pagetext']));
                            $htmlContent = $bbcode_parser->parse($lastPost['pagetext'], $oMbqEtForumTopic->forumId->oriValue);
                            $oMbqEtForumTopic->shortContent->setOriValue(MbqMain::$oMbqCm->getShortContent(MbqMain::$oMbqCm->exttNativeStrToUtf8($oMbqRdEtForumPost->processContentForDisplay($htmlContent, true))));  //!!!
            	            break;
            	        }
            	    }
            	}
     ...
     * @param  Mixed  $var
     * @param  Array  $mbqOpt
     * $mbqOpt['case'] = 'topicRecord' means init forum topic by topicRecord
    ...
    public function initOMbqEtForumTopic($var, $mbqOpt) {
        ...
        eval(ExttMbqGlobal::$v);
        /* global common end */
        if ($mbqOpt['case'] == 'topicRecord') {
            $oMbqEtForumTopic = MbqMain::$oClk->newObj('MbqEtForumTopic');
            $oMbqEtForumTopic->totalPostNum->setOriValue($var['totalposts']);
            $oMbqEtForumTopic->topicId->setOriValue($var['threadid']);
            $oMbqEtForumTopic->forumId->setOriValue($var['forumid']);
            $oMbqEtForumTopic->firstPostId->setOriValue($var['firstpostid']);
            $oMbqEtForumTopic->topicTitle->setOriValue(MbqMain::$oMbqCm->exttNativeStrToUtf8($var['threadtitle']));
            $oMbqEtForumTopic->topicAuthorId->setOriValue($var['postuserid']);
            $oMbqEtForumTopic->postTime->setOriValue($var['lastpost']);
            $oMbqEtForumTopic->lastReplyTime->setOriValue($var['lastpost']);
            $oMbqEtForumTopic->replyNumber->setOriValue($var['replycount']);
            $oMbqEtForumTopic->viewNumber->setOriValue($var['views']);
            if ($var['sticky'] == 1) {
