...
Class MbqRdEtForumPost extends MbqBaseRdEtForumPost {
     ...
     * $mbqOpt['case'] = 'byPostRecords' means get data by postRecords.$var is the postRecords.
     * @return  Mixed
     */
    public function getObjsMbqEtForumPost($var, $mbqOpt) {
        ...
        if ($mbqOpt['case'] == 'byTopic') {
            ...
            if ($mbqOpt['oMbqDataPage']) {
                ...
                //ref showthread.php START MAIN SCRIPT
// #######################################################################
// ######################## START MAIN SCRIPT ############################
// #######################################################################
$threadid = $var->topicId->oriValue;    //wztmdf 20131008
...
$vbulletin->GPC['pagenumber'] = $oMbqDataPage->curPage;   //wztmdf 20131008
...
if ($vbulletin->options['allowthreadedmode'] AND !$show['search_engine'])
{
	...
	$threadedmode = 0;  //wztmdf 20131008
...
$perpage = $oMbqDataPage->numPerPage;   //wztmdf 20131008
...
// get thread info
//$thread = verify_id('thread', $threadid, 1, 1);
if (!($thread = verify_id('thread', $threadid, 0, 1))) MbqError::alert('', 'Can not find topic:'.$threadid.'.');    //wztmdf 20131008
$threadinfo =& $thread;
...
if (!verify_forum_password($foruminfo['forumid'], $foruminfo['password'], false)) { //wztmdf 20131009
    MbqError::alert('', 'Can not access password forum:'.$foruminfo['forumid'].'.');
}
...
if ($threadedmode == 0)
{
	...
	$oMbqDataPage->totalNum = $totalposts;    //wztmdf 20131009

	// load attachments
	if ($thread['attach'])
	{
		$attachments = $db->query_read("
			SELECT userid, dateline, thumbnail_dateline, filename, filesize, visible, attachmentid, counter,
...
//eval('print_output("' . fetch_template('SHOWTHREAD') . '");');
                /* common begin */
                $mbqOpt['case'] = 'byPostRecords';
                $mbqOpt['oMbqDataPage'] = $oMbqDataPage;
                return $this->getObjsMbqEtForumPost($exttMbqPostRecords, $mbqOpt);
                /* common end */
            }
        } elseif ($mbqOpt['case'] == 'byPostRecords') {
            $arrPostRecord = $var;
            /* common begin */
            ...
            foreach ($objsMbqEtForumPost as &$oMbqEtForumPost) {
                if ($oMbqEtForumPost->mbqBind['postRecord']['attachments']) {
                    foreach ($oMbqEtForumPost->mbqBind['postRecord']['attachments'] as $attachmentRecord) {
                        $oMbqEtForumPost->objsMbqEtAtt[] = $oMbqRdEtAtt->initOMbqEtAtt($attachmentRecord, array('case' => 'attachmentRecord'));
                    }
                }
            }
        	...
        	foreach ($objsMbqEtForumPost as &$oMbqEtForumPost) {
        	    $tempPageText = $oMbqEtForumPost->mbqBind['postRecord']['pagetext'];
                $oMbqEtForumPost->postContent->setOriValue(MbqMain::$oMbqCm->exttNativeStrToUtf8($tempPageText));
                //$htmlContent = $bbcode_parser->parse($tempPageText, $oMbqEtForumPost->forumId->oriValue);
                $htmlContent = $oMbqEtForumPost->mbqBind['postRecord']['message'];
                $oMbqEtForumPost->postContent->setAppDisplayValue(MbqMain::$oMbqCm->exttNativeStrToUtf8($htmlContent));
                $oMbqEtForumPost->postContent->setTmlDisplayValue(MbqMain::$oMbqCm->exttNativeStrToUtf8($this->processContentForDisplay($htmlContent, true, $oMbqEtForumPost)));
                $oMbqEtForumPost->postContent->setTmlDisplayValueNoHtml(MbqMain::$oMbqCm->exttNativeStrToUtf8($this->processContentForDisplay($htmlContent, false, $oMbqEtForumPost)));
    ...
    public function initOMbqEtForumPost($var, $mbqOpt) {
        ...
        if ($mbqOpt['case'] == 'postRecord') {
            ...
            $oMbqEtForumPost->postId->setOriValue($var['postid']);
            $oMbqEtForumPost->parentPostId->setOriValue($var['parentid']);
            $oMbqEtForumPost->topicId->setOriValue($var['threadid']);
            $oMbqEtForumPost->postTitle->setOriValue(MbqMain::$oMbqCm->exttNativeStrToUtf8($var['title']));
            $oMbqEtForumPost->postAuthorId->setOriValue($var['userid']);
            $oMbqEtForumPost->postTime->setOriValue($var['dateline']);
     ...
     *
     * @param  Mixed  $var content string
    ...
    public function processContentForDisplay($var, $returnHtml, $obj = NULL) {
        ...
        */
        ...
        if ($returnHtml) {
            //handle quote
            $post = preg_replace('/<div style="margin:20px; margin-top:5px; ">.*?<div class="smallfont" style="margin-bottom:2px">Quote:<\/div>.*?<table cellpadding="6" cellspacing="0" border="0" width="100%">.*?<tr>.*?<td class="alt2" style="border:1px inset">.*?<div>.*?Originally Posted by <strong>(.*?)<\/strong>.*?<a href="[^>]*?" rel="nofollow"><img class="inlineimg" src="[^>]*?" border="0" alt="View Post" \/><\/a>.*?<\/div>.*?<div style="font-style:italic">(.*?)<\/div>.*?<\/td>.*?<\/tr>.*?<\/table>.*?<\/div>/is', '$1 wrote:[quote]$2[/quote]', $post);
            //handle simple quote
            $post = preg_replace('/<div style="margin:20px; margin-top:5px; ">.*?<div class="smallfont" style="margin-bottom:2px">Quote:<\/div>.*?<table cellpadding="6" cellspacing="0" border="0" width="100%">.*?<tr>.*?<td class="alt2" style="border:1px inset">(.*?)<\/td>.*?<\/tr>.*?<\/table>.*?<\/div>/is', '[quote]$1[/quote]', $post);
            //image
	        //$post = preg_replace('/\[IMG\]([^\[]*?)\[\/IMG\]/i', '[img]$1[/img]', $post);   //inserted image through web page content editor
	        $post = preg_replace('/<img [^>]*?src="(images\/.*?)" [^>]*?\/>/i', '[img]'.MbqMain::$oMbqAppEnv->rootUrl.'$1[/img]', $post);  //smilies
	        ...
	        $post = preg_replace_callback('/\[url=([^\]]*?)\]([^\[]*?)\[\/url\]/i', create_function('$matches','return ((substr($matches[1], 0, 7) == "http://") || (substr($matches[1], 0, 8) == "https://")) ? "[url=".$matches[1]."]".$matches[2]."[/url]" : "[url=".MbqMain::$oMbqAppEnv->rootUrl."$matches[1]]$matches[2][/url]";'), $post);
	        //code
            $post = preg_replace('/<div style="margin:20px; margin-top:5px">[^<]*?<div class="smallfont" style="margin-bottom:2px">Code:<\/div>.*?<pre class="alt2" dir="ltr" [^>]*?>(.*?)<\/pre>.*?<\/div>/is', '[quote]$1[/quote]', $post);
	        //html code
            $post = preg_replace('/<div style="margin:20px; margin-top:5px">[^<]*?<div class="smallfont" style="margin-bottom:2px">HTML Code:<\/div>.*?<pre class="alt2" dir="ltr" [^>]*?>(.*?)<\/pre>.*?<\/div>/is', '[quote]$1[/quote]', $post);
	        //php code
            $post = preg_replace('/<div style="margin:20px; margin-top:5px">[^<]*?<div class="smallfont" style="margin-bottom:2px">PHP Code:<\/div>.*?<div class="alt2" dir="ltr" [^>]*?>.*?<code style="white-space:nowrap">.*?<code><span style="color: #000000">.*?<span style="color: #0000BB">(.*?)<\/span>.*?<\/span>.*?<\/code>.*?<\/code>.*?<\/div>.*?<\/div>/is', '[quote]$1[/quote]', $post);
            //blockquote
    	    $post = str_ireplace('<blockquote>', "<br />\t\t<blockquote>", $post);
    	    $post = str_ireplace('</blockquote>', "</blockquote><br />", $post);
    	    //highlight
	        $post = preg_replace('/<span class="highlight">([^<]*?)<\/span>/i', '<font color="red">$1</font>', $post);
