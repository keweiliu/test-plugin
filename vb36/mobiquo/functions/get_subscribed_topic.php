	...
	'dostuff' => array(
		...
		");
			...
			while ($thread = $db->fetch_array($threads))
			{
				...
				$lastuserinfo = mobiquo_verify_id('user',$last_topic['userid'], 1, 1, $fetch_userinfo_options);
				...
				$return_topic = new xmlrpcval(array('forum_id'=>new xmlrpcval($thread[forumid],"string"),
                                                  ...
                                                  'post_time'=>new xmlrpcval(mobiquo_iso8601_encode($thread[lastpost]-$vbulletin->options['hourdiff'],$vbulletin->userinfo['tzoffset']),"dateTime.iso8601")),"struct");  
