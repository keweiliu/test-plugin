...
function reply_topic_func($xmlrpc_params) {
	...
	$reply_threadid = intval($decode_params[0]);
	//$reply_postid = $decode_params[1];
	$reply_title = "";
	$reply_message = in_text_clean(mobiquo_encode($decode_params[2],'to_local'));

	//$vbulletin->GPC['threadid'] = $reply_threadid;
		...
		else
		{
			...
			if ($newpost['postid']==0){
				...
				array(
											...
											'post_id' => new xmlrpcval($newpost['postid'],'string'),
				),
									'struct'
									)
									);
