...
function get_thread_func($xmlrpc_params){
	...
	if(isset($params[1]) && $params[1] >= 0) {
	 ...
	 return get_thread_content($threadid,$start_num,$end_num,-1,$html_content);
}
...
function get_thread_by_unread_func($xmlrpc_params){
	...
	if(isset($params[1]) && $params[1] >= 0) {
		...
		if(!is_array($threadinfo) || !$threadinfo['threadid']){
	...
	");
		...
		return get_thread_content($threadid,$limitlower,$limitupper,$postid, $html_content);
}
...
function get_thread_content($threadid,$start_num,$end_num,$postid = -1,$html_content = false){
	global $db;
	global $vbulletin;
	...
	");
		...
		while ($post = $db->fetch_array($posts))
		{
			...
			if ( $fetchtype == 'post' OR 	($fetchtype == 'post_deleted' AND $show['viewpost']))
			{
				...
				if($html_content){
					...
					$post_content =  post_content_clean_html($post_content);
					...
					$post_content = mobiquo_encode($post_content);
		...
		array(
										...
										'is_approved'    =>  new xmlrpcval($is_approved ,"boolean"),
										'is_sticky'      => new xmlrpcval($threadinfo[sticky],"boolean"),
										'can_approve'    => new xmlrpcval($show['moderatethread'],"boolean"),
