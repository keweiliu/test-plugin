...
function get_participated_topic_func($xmlrpc_params){
	...
	if($mobqiuo_search_id){
		return get_search_result($mobqiuo_search_id,$start_num,$end_num,true,true);
	}
	// #############################################################################
	...
	$vbulletin->GPC['searchuser']= $user_name;
	...
	if (empty($errors))
	{
		...
		if (empty($errors))
		{
			...
			$db->query_write("
				REPLACE INTO " . TABLE_PREFIX . "search
					(userid, titleonly, ipaddress, personal, query, searchuser, forumchoice, sortby, sortorder, searchtime, showposts, orderedids, dateline, searchterms, displayterms, searchhash, completed)
				VALUES (
					" . $vbulletin->userinfo['userid'] . ",
					" . intval($vbulletin->GPC['titleonly']) . " ,
					'" . $db->escape_string(IPADDRESS) . "',
				...
				if (empty($errors))
				{
					...
					else
					{
						...
						");
								...
								return get_search_result($searchid,$start_num,$end_num,true,false);
					}
				} else {
