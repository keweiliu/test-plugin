...
function return_fault($params)
{
    ...
    $response = new xmlrpcresp(
        new xmlrpcval(array(
            ...
            'result_text'   => new xmlrpcval($faultString, 'base64'),
        ),'struct')
    );
...
function post_content_clean($str){
	$bbcode_array = array('B','I','U','COLOR','SIZE','FONT','HIGHLIGHT','INDENT','LEFT','RIGHT','CENTER','THREAD','POST','LIST','CODE','PHP','HTML','NOPARSE','ATTACH','BUG','SCREENCAST');
	...
	$str = preg_replace ("/(\[email.*?\])(.*?)(\[\/email\])/si","[URL]$2[/URL]",$str);


	$str = preg_replace ("/(\[shot\])(.*)(\[\/shot\])/siU","[IMG]$2[/IMG]",$str);
...
function post_content_clean_html($str){
	$bbcode_array = array('SIZE','FONT','HIGHLIGHT','LEFT','RIGHT','CENTER','THREAD','POST','LIST','CODE','PHP','HTML','NOPARSE','ATTACH','BUG','SCREENCAST');
	...
	if (is_dir($dir)) {
		if ($dh = opendir($dir)) {
			while (($file = readdir($dh)) !== false) {
				if(preg_match_all("/global\.php(\d*)/",$file,$r)){
					$suffix =  $r[1][0];
				}
			}
			closedir($dh);
