			...
			");
					...
					while ($user = $this->registry->db->fetch_array($friends_sql))
					{
						...
						if($authorinfo[avatarurl]){
							$icon_url=get_icon_real_url($user['avatarurl']);
						} else {
							$icon_url = '';
						}
			   ...
			   if($loggedin['where']){
		  		...
		  		$return_user = new xmlrpcval(
		  		array( 'user_name'=>new xmlrpcval($user[username],'base64'),
						'icon_url'=>new xmlrpcval(	$icon_url,'string'),
						'is_online'=>new xmlrpcval($mobiquo_user_online,'boolean'),
						'display_text'=>new xmlrpcval($display_text,'string'),
						'last_active_time'=>new xmlrpcval(mobiquo_iso8601_encode($thread[lastactivity]-$vbulletin->options['hourdiff'],$vbulletin->userinfo['tzoffset']),'dateTime.iso8601')
		  		)   , 'struct'
		  		);

		  		$this->mobiquo_array[] = $return_user;
					}
...
class vB_ProfileBlock_Groups extends vB_ProfileBlock
{
	...
	function prepare_output($id = '', $options = array())
	{
		...
		$this->block_data['show_join_link'] = ((!empty($this->block_data['socialgroupbits']) OR $this->profile->prepared['myprofile']) AND $this->registry->userinfo['permissions']['socialgrouppermissions'] & $this->registry->bf_ugp_socialgrouppermissions['canjoingroups']) ? true : false;
	}
}
...
/*======================================================================*\
 || ####################################################################
 || # Downloaded: 23:24, Wed Apr 7th 2010
 || # CVS: $RCSfile$ - $Revision: 35340 $
 || ####################################################################
 \*======================================================================*/
?>
