			...
			if (!empty($post[attachments]["$attachmentid"]))
			{
				...
				switch($attachment['extension'])
				{
					...
					case 'pdf':
						...
						break;
					default:
						$replace[] = "<a href=\"{$vbulletin->options['bburl']}/attachment.php?{$vbulletin->session->vars['sessionurl']}attachmentid=\\1&amp;d=$attachment[dateline]\" $addtarget>$attachment[filename]</a>";
				}
			}
			else
