...
class tapatalk_usercp extends ipsCommand
{
    public $result = true;
    public $result_text = '';
    ...
    public function doExecute( ipsRegistry $registry )
    {
        ...
        {
            get_error('You must be logged to do it');
            /*
        ...
        // tapatalk code
        if (isset($_POST['tt_token']))
        {
            $response = tt_register_verify($_POST['tt_token'], $_POST['tt_code']);
            if($response->result == false)
                get_error($response->result_text);
            
            if ($this->memberData['email'] != $response->email)
                get_error('Unmatched user');
        ...
        else if ( $_FUNC == 'saveForm' )
        {
            ...
            */
                if ($this->request['uid'])
                {
                    $member = IPSMember::load( $this->request['uid'], 'core', 'id' );
                    if ($member)
                    {
                        $this->request['newbox_1'] = $member['members_display_name'];
                        $_POST['newbox_1'] = $member['members_display_name'];
                    }
                }
                ...
                {
                    $this->registry->getClass('output')->showError( $errors[0] );
                    //$html = $usercp_module->showForm( $_AREA, $errors );
                }
                else if ( $usercp_module->ok_message )
                {
                    $this->result_text = $usercp_module->ok_message;
