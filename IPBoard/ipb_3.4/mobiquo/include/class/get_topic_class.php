...
class forum_topic extends public_forums_forums_forums
{
    public function doExecute( ipsRegistry $registry )
    {
        ...
        else
        {
            ...
            if (!empty($data['topic_data']))
            {
                ...
                $parser->set( array( 'parseArea'      => 'topics',
                                     ...
                                     'parseBBCode'    => $this->registry->class_forums->forum_by_id[ $this->forum['id'] ]['use_ibc'],
                                     'parseHtml'      => 0,
                                     'parseEmoticons' => 1 ) );
                foreach($data['topic_data'] as $topic)
                {
                    $post = $this->registry->getClass('topics')->getPostById( $topic['topic_firstpost'] );
                    $post['post'] = preHtmlToBBCode($post['post']);
                    $post['post'] = $parser->HtmlToBBCode( $post['post'] );
                    $post['post'] = IPSText:: br2nl($post['post']);
                    ...
                    $is_moved = $topic['folder_img']['is_moved'] ? true : false;
                    
                    $can_rename = $can_stick = $can_close = $can_move = $can_merge = $can_approve = $can_delete = false;
                    if (!$topic['_isArchived'])
                    {
                        if ($this->memberData['g_is_supmod']) {
                            $can_rename = $can_stick = $can_close = $can_move = $can_merge = $can_approve = $can_delete = true;
                    ...
                    $xmlrpc_topic = array(
                        ...
                        'topic_id'          => new xmlrpcval(isset($topic['real_tid']) ? $topic['real_tid'] : $topic['tid'], 'string'),
                        ...
                        'short_content'     => new xmlrpcval(get_short_content($preview,0,500), 'base64'),
                    ...
                    if ($topic['tags']['prefix'])
                    {
                        $xmlrpc_topic['prefix']     = new xmlrpcval(subject_clean($topic['tags']['prefix']), 'base64');
                        $xmlrpc_topic['prefix_id']  = new xmlrpcval(subject_clean($topic['tags']['prefix']));
                    }
                    ...
                    if ($is_moved)      $xmlrpc_topic['is_moved']       = new xmlrpcval(true, 'boolean');
                    if ($topic['real_tid'] != $topic['tid'])
                                        $xmlrpc_topic['real_topic_id']  = new xmlrpcval($topic['tid']);
                    ...
                    if ($can_merge)     $xmlrpc_topic['can_merge']      = new xmlrpcval(true, 'boolean');
        ...
        $where = array( 'meta_parent_id'  => intval( $this->forum['id'] ),
                        'member_id'       => $this->memberData['member_id'] );
        
        if ( $this->registry->tags->can( 'add', $where ) && !$this->settings['tags_open_system'] && $this->settings['tags_predefined'])
        {
            ...
            $topics['require_prefix'] = $this->settings['tags_min'] and $this->settings['tags_min_req'];
            $topics['prefixes'] = $final;
        }
