...
class tapatalk_modcp extends ipsCommand
{
    public $result;
    ...
    protected function _setAsSpammer()
    {
        ...
        {
            get_error( 'mod_cannot_edit_admin');
        }
    ...
    public function executePlugin( $permissions )
    {
        ...
        if( count( $topics ) )
        {
            foreach( $topics as $tid => $topic )
            {
                ...
                $topic['is_ban']  = $is_spam;
                $topic['can_ban'] = $can_mark_spam;
    ...
    public function executePlugin( $permissions )
    {
        ...
        if( count( $posts ) )
        {
            foreach( $posts as $pid => $item )
            {
                ...
                $preview = IPSText::getTextClass( 'bbcode' )->stripQuotes( $data['post'] );
                $preview = IPSText::getTextClass( 'bbcode' )->preDisplayParse( $preview );
                $text    = IPSText::getTextClass( 'bbcode' )->stripAllTags( strip_tags( $preview, '<br>' ) );
                $preview = IPSText::truncate($text , 200 );
                $post['preview'] = $preview;
                
                $data = IPSMember::buildDisplayData( $post );
                $data['post'] = str_replace('/<#EMO_DIR#>/', '/#EMO_DIR#/', $data['post']);
                $data['post'] = preg_replace('/<img [^>]*?class=(\'|")bbc_emoticon\1 [^>]*?alt=(\'|")(.*?)\2[^>]*?\/?>/si', '$3', $data['post']);
                if ($data['post_htmlstate'])
                {
                ...
                $post['post_content'] = post_bbcode_clean($data['post']);
