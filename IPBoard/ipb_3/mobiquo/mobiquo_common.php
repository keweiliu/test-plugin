...
function post_html_clean($str)
{
    ...
    // remove quote icon
    $str = preg_replace('/\[img\](.*)(snapback.png|attachicon.gif)\[\/img\]/siU', '', $str);
...
function parse_media($str)
{
    ...
    $str = preg_replace('/<iframe .*?src="(.*?)".*?\/><\/iframe>/si', "[url=$1]$1[/url]", $str);    //fixed bug:can not display youtube link in ipb 3.4.2 version.
...
function post_bbcode_clean($str)
{
    ...
    $array_reg = array(
        ...
        array('reg' => '/\[color=((\'|"|&#39;|&quot;)?)(.*?)\1\](.*?)\[\/color\]/sei','replace' => "mobi_color_convert('$3','$4' ,false)"),
