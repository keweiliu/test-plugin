...
if (defined('TT_PUSH_TYPE') && !defined('mobiquo_push_sent'))
{
    ...
    if (TT_PUSH_TYPE == 'pm')
    {
        if($vbulletin->db->query_first("SELECT * FROM " . TABLE_PREFIX . "tapatalk_users WHERE userid='$user[userid]'"))
        {
        ...
        if ($post['visible'] AND !in_coventry($vbulletin->userinfo['userid'], true) AND $post['postid'] AND $threadinfo['threadid'])
        {
            ...
            {//Add new_topic push data
                if($threadinfo['forumid'])
                {
                    ...
                    {
                        foreach($new_topic_users as $new_topic_users)
                        {
                            if($vbulletin->userinfo['userid'] == $new_topic_users)
                                continue;   // Don't send to author himself.
                            $push_data[] = array(
                                'userid'    => $new_topic_users,
                                'type'      => 'newtopic',
                                'id'        => $threadid,
                                'subid'     => $postid,
                                'title'     => tt_hook_encode($push_title),
            ...
            if (!empty($push_data))
            {
                ...
                foreach($push_data as $item)
                {
                    $vbulletin->db->query_write("
                        ...
                        VALUES
                            ('$item[userid]', '$item[type]', '$threadid', '$postid', '$ttp_title', '$ttp_author', '$item[dateline]')"
                    );
                }
            }
        }
