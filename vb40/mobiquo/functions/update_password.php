...
function update_password_func($xmlrpc_params)
{
    ...
    if(isset($params[2]))
    {
        ...
        if( (!isset($user_emaill) || empty($user_emaill)) && (!isset($email_response['email']) || empty($email_response['email'])))
            return return_fault('You need to input an email or re-login tapatalk id to use default email of tapatalk id.');
    ...
    else if (!isset($tapatalkid_email) && $userdata->hash_password($userdata->verify_md5($vbulletin->GPC['currentpassword_md5']) ? $vbulletin->GPC['currentpassword_md5'] : $vbulletin->GPC['currentpassword'], $vbulletin->userinfo['salt']) != $vbulletin->userinfo['password'])
    {
