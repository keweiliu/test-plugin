    ...
    if (!empty($emailupdate))
    {
        ...
        foreach ($vbulletin->forumcache AS $forumid => $forum)
        {
            if ($forum['subscribeforumid'] != '')
            {
                ...
                $new_post = $forum['statusicon'] == 'new' ? true : false;
                ...
                if (!$cancontainthreads)    $forumxmlrpc['sub_only']     = new xmlrpcval(true, 'boolean');
                if ($forum['password'])     $forumxmlrpc['is_protected'] = new xmlrpcval(true, 'boolean');
                if ($new_post)              $forumxmlrpc['new_post']     = new xmlrpcval(true, 'boolean');
                
                $forumsinfo[] = new xmlrpcval($forumxmlrpc, 'struct');
            }
        }
    }
